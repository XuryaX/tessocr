...
*/
	...
	*/
	...
	LinkedList<TextRegion> discardNonText( LinkedList<TextRegion> boxes, int[][] contrast ) {
	...
	*/
	...
	/**
	 * @uml.property  name="merge_densityFactor"
	 */
	public double merge_densityFactor;
	
	/**
	 * @uml.property  name="merge_mass"
	 */
	public int merge_mass;
	
	/**
	 * @uml.property  name="merge_dist1"
	 */
	public int merge_dist1;
	
	/**
	 * @uml.property  name="merge_distfac"
	 */
	public double merge_distfac;
	
	/**
	 * @uml.property  name="merge_dist2"
	 */
	...
	boolean merge( TextRegion thisBox, TextRegion thatBox ) {
	 ...
	 if( Math.abs(thisBox.vstart() - thatBox.vstart()) < merge_dist1
	     ...
	     && (Math.abs(thisBox.hstart()-thatBox.hend()) < merge_distfac * maxboxwidth
	...
	* @return a LinkedList<TextRegion> of boxes that are likely to contain text.
	...
	*/
	...
	private void setimg(BufferedImage img,int width,int height, int r,int g,int bl){
	...
	private void setimg(BufferedImage img,BufferedImage src,int width,int height,boolean mono){
		int r,g,bl;
		for( int a = 0; a < width; a++ ){
	 ...
	 */
	private void setimg(BufferedImage img,BufferedImage src,int width,int height,int xstart,int xend,int ystart,int yend,boolean mono){
		int r,g,bl;
		int x= xstart;
		int y;
	...
	/**
	 * Copy Regions of text from source image to arrays of Sliced Buffered Images
	 */
		...
		ArrayList<TextRegion> boxes = new ArrayList<TextRegion>(lbox);
		 ...
		 for( int i = 0; i < boxes.size(); i++ ) {
		     ...
		     int height = thisBox.height()+50;
			   ...
			   setimg(outputx,width,height,255,255,255);//Set white background
		     ...
		     setimg(outputx,image,outputx.getWidth(),outputx.getHeight(),x1,x2,y1,y2,true); //Copy current text box to this image snippet
	...
	public BufferedImage SetRedBorder(LinkedList<TextRegion> lbox)
	{
		...
		BufferedImage outputimage
		 ...
		 setimg(outputimage,image,image.getWidth(),image.getHeight(),true);  // make everything monochrome
